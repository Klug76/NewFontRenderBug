...
set ICONS=%AND_ICONS%
set PLATFORM_SDK=-platformsdk %ANDROID_SDK%
if "%TARGET%"=="aab" goto aab
set DIST_EXT=apk
set TYPE=apk
goto start
:aab
...
set OUTPUT=%DIST_PATH%\%DIST_NAME%-rc.%DIST_EXT%
::if "%OPTIONS%"=="-arch x86" set OUTPUT=%DIST_PATH%\%DIST_NAME%-rc-x86.%DIST_EXT%
...
echo.
@echo on
call adt -licenseDevID f8ea7deb-b55e-47ac-bc84-1daef987ef6e -package -target %TYPE%%TARGET% %OPTIONS% %SIGNING_OPTIONS% "%OUTPUT%" "%APP_XML%" %FILE_OR_DIR% %EXT_OPTIONS% %PLATFORM_SDK%
@echo off
echo.
